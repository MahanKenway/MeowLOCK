...
export default function App() {
  ...
  // Coordinated load of IndexedDB, background assets, and workspace states on mount
  useEffect(() => {
    let active = true;
    const minDelay = new Promise((resolve) => setTimeout(resolve, 1800));

    const loadAssets = async () => {
      let resolvedBgUrl: string | null = null;
      let resolvedBgType: "image" | "video" = "image";

      try {
        const res = await loadBgFromIndexedDB();
        if (res && active) {
          setLocalBgUrl(res.url);
          setLocalBgType(res.type);
          resolvedBgUrl = res.url;
          resolvedBgType = res.type;
        }
      } catch (err) {
        console.error("Failed to load background from IndexedDB:", err);
      }

      // Preload active background if it is an image
      const isLocal = activeProfile.bgUrl === "local";
      const targetBgUrl = isLocal ? resolvedBgUrl : activeProfile.bgUrl;
      const targetBgType = isLocal ? resolvedBgType : (activeProfile.bgUrl && (
      ...
      if (targetBgUrl && targetBgType === "image") {
      ...
      // Wait for at least the minimum delay so the beautiful loader animations finish smoothly
      await minDelay;
      if (active) {
  ...
  return (
    ...
    >
      ...
      {/* Absolute Fullscreen Glassmorphic Splash Screen & Skeletal Loader */}
      <AnimatePresence>
        {isAppLoading && (
          <motion.div
            key="workspace-splash-screen"
            initial={{ opacity: 1 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          >
            ...
            <div className="max-w-md w-full flex flex-col items-center px-6 text-center z-10">
              {/* Header Icon Ring */}
              <motion.div 
                initial={{ scale: 0.8, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                transition={{ duration: 0.6, delay: 0.2 }}
                className="w-16 h-16 rounded-2xl border border-[#7c3aed]/30 bg-[#7c3aed]/5 flex items-center justify-center mb-6 shadow-lg shadow-[#7c3aed]/10 relative overflow-hidden"
              >
                <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-[#7c3aed]/10 to-transparent animate-pulse" />
              ...
              <motion.h1 
                ...
                className="text-2xl font-semibold tracking-[0.25em] uppercase font-sans text-neutral-100"
              >
                Zen Workspace
              </motion.h1>
              ...
              <motion.p 
                ...
                className="text-sm font-mono tracking-wide text-neutral-400 mt-2"
              >
                PREPARING YOUR COZY FOCUS ENVIRONMENT
              </motion.p>

              {/* Skeletal Blueprint Visualization */}
              <motion.div 
                initial={{ y: 20, opacity: 0 }}
                animate={{ y: 0, opacity: 1 }}
                transition={{ duration: 0.8, delay: 0.5 }}
                className="w-full mt-10 p-5 rounded-2xl border border-white/5 bg-neutral-900/40 backdrop-blur-md flex flex-col gap-4 relative overflow-hidden shadow-2xl"
              >
                ...
                <div className="flex items-center justify-between border-b border-white/5 pb-3">
                  ...
                  <div className="flex gap-1.5">
                    <div className="h-2 w-2 bg-white/15 rounded-full" />
                    <div className="h-2 w-2 bg-[#7c3aed]/40 rounded-full animate-pulse" />
                    <div className="h-2 w-2 bg-white/15 rounded-full" />
                  </div>
                </div>

                {/* Wireframe Layout representation */}
                <div className="grid grid-cols-3 gap-3 h-32 items-center justify-center">
                  {/* Left Dock Widget Skeletal Card */}
                  <div className="h-full border border-white/5 bg-white/[0.02] rounded-xl p-3 flex flex-col gap-3 justify-center animate-pulse">
                    <div className="h-2.5 w-11/12 bg-white/10 rounded-full" />
                    <div className="h-2 w-7/12 bg-white/10 rounded-full" />
                    <div className="h-2 w-9/12 bg-[#7c3aed]/15 rounded-full" />
                  </div>

                  {/* Centered Large Interactive Clock Skeletal Card */}
                  <div className="h-full border border-white/5 bg-white/[0.01] rounded-xl p-3 flex flex-col items-center justify-center relative overflow-hidden">
                    <div className="h-16 w-16 rounded-full border border-dashed border-[#7c3aed]/30 flex items-center justify-center animate-[spin_12s_linear_infinite] relative">
                      <div className="h-2 w-2 bg-[#7c3aed]/40 rounded-full absolute top-1" />
                    </div>
                    <div className="h-2 w-10 bg-white/10 rounded-full mt-3 animate-pulse" />
                  </div>

                  {/* Right Dock Widget Skeletal Card */}
                  <div className="h-full border border-white/5 bg-white/[0.02] rounded-xl p-3 flex flex-col gap-3 justify-center animate-pulse [animation-delay:0.3s]">
                    <div className="h-2.5 w-8/12 bg-white/10 rounded-full" />
                    <div className="h-2 w-11/12 bg-white/10 rounded-full" />
                    <div className="h-2 w-1/2 bg-[#7c3aed]/15 rounded-full" />
                  </div>
                </div>
              </motion.div>

              {/* Progress Tracking Loading Line */}
              <motion.div 
                initial={{ scaleX: 0.8, opacity: 0 }}
                animate={{ scaleX: 1, opacity: 1 }}
                transition={{ duration: 0.6, delay: 0.6 }}
                className="w-full bg-white/5 h-1 rounded-full mt-10 overflow-hidden relative border border-white/5"
              >
                <motion.div
                  initial={{ left: "-100%" }}
                  animate={{ left: "100%" }}
                  transition={{ repeat: Infinity, duration: 1.8, ease: "easeInOut" }}
                  className="absolute top-0 bottom-0 w-1/3 bg-gradient-to-r from-transparent via-[#7c3aed] to-transparent"
                />
              </motion.div>
              ...
              >
                ...
                <span className="text-[11px] font-mono tracking-widest text-neutral-400 uppercase">
                  Assembling Widgets & Backgrounds
                </span>
              </motion.div>
            </div>
          </motion.div>
